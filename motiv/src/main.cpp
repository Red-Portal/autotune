...
int main()
{
    ...
    auto m_seq = {4, 16, 64, 256, 1024, 4096, 16384};
    auto n_seq = {4, 16, 64, 256, 1024, 4096, 16384};
    auto k_seq = {4, 16, 64, 256, 1024, 4096, 16384};
    ...
    auto outstream = std::ofstream("data.json");
	...
	for (auto thread : t_seq)
	{
		...
		for (auto n : n_seq) {
			...
			outstream << std::setw(2) << log << std::endl;
			outstream.flush();
		    }
		}
	    }
